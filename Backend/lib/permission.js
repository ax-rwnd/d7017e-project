...
function checkIfTestInAssignment(assignment_id, test_id) {
    return Assignment.find({ _id: assignment_id, 'tests.io': {$in: [test_id]} }).count().then(function (count) {
        if (count === 0) {
...
exports.checkIfTestInAssignment = checkIfTestInAssignment;
